...
function createAlert (title, summary, details, severity, dismissible, autoDismiss) {
  function opacity () {
    if (document.getElementById('pageMessages').hasChildNodes()) {
      // document.querySelector('.app-container').classList.remove('backgroundOut')
      // document.querySelector('.app-container').classList.add('backgroundIn')
    } else {
      // document.querySelector('.app-container').classList.remove('backgroundIn')
      // document.querySelector('.app-container').classList.add('backgroundOut')
    }
  }
